...
export type BaseDataMap =
  ...
  { /** 打开开发工具           */[name in PARSE_NAME["openDevTools"]]: undefined; } &
  { /** 主窗口_最小化          */[name in MAIN_WINDOW["min"]]: undefined; } &
  { /** 主窗口_最大化          */[name in MAIN_WINDOW["max"]]: undefined; } &
  { /** 主窗口_切换最小化      */[name in MAIN_WINDOW["min_toggle"]]: undefined; } &
  { /** 主窗口_切换隐藏        */[name in MAIN_WINDOW["hide_toggle"]]: undefined; } &
  { /** 主窗口_关闭            */[name in MAIN_WINDOW["close"]]: boolean; } &
  { /** 主窗口_退出            */[name in MAIN_WINDOW["quit"]]: undefined; } &
  { /** 主窗口_聚焦            */[name in MAIN_WINDOW["focus"]]: undefined; } &
  { /** 主窗口_重置窗体         */[name in MAIN_WINDOW["restart_window"]]: LxMusic.Common.KeyNames; } &// TODO 研发中
  ...
  { /** 主窗口_清除歌词         */[name in MAIN_WINDOW["clear_lyric"]]: undefined; } &
  ...
  { /** 主窗口_清除歌曲地址     */[name in MAIN_WINDOW["clear_music_url"]]: undefined; } &
  ...
  { /** 主窗口_退出更新         */[name in MAIN_WINDOW["quit_update"]]: undefined; } &
...
export type MainHandleDataMap =
  ...
  { /** 热键_取状态                 */[name in HOT_KEY["status"]]: ParamRet<undefined, LxMusic.HotKey.HotKeyMap>; } &
  ...
  { /** 主窗口_获取数据             */[name in MAIN_WINDOW["get_data"]]: ParamRet<string, unknown>; } &
  ...
  { /** 主窗口_取配置               */[name in MAIN_WINDOW["get_setting"]]: ParamRet<undefined, LxMusic.Renderer.MainSettingInfo>; } &
  { /** 主窗口_清除缓存             */[name in MAIN_WINDOW["clear_cache"]]: ParamRet<undefined, void>; } &
  { /** 主窗口_取缓存大小           */[name in MAIN_WINDOW["get_cache_size"]]: ParamRet<undefined, number>; } &
  { /** 主窗口_取数据路径           */[name in MAIN_WINDOW["get_data_path"]]: ParamRet<undefined, string>; } &
  { /** 主窗口_取环境参数           */[name in MAIN_WINDOW["get_env_params"]]: ParamRet<undefined, NodeJS.Global["envParams"]["cmdParams"]>; } &
  ...
  { /** 主窗口_取用户网络接口列表   */[name in MAIN_WINDOW["get_user_api_list"]]: ParamRet<undefined, LxMusic.UserApi.ApiInfo[]>; } &
  { /** 主窗口_取用户网络接口状态   */[name in MAIN_WINDOW["get_user_api_status"]]: ParamRet<undefined, LxMusic.UserApiEvent.InitInfo>; } &
  { /** 歌词窗口_获取歌词配置       */[name in LYRIC_WINDOW["get_lyric_config"]]: ParamRet<undefined, LxMusic.WinLyric.LyricConfigInfo>; };
