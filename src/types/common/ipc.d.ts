...
export type BaseDataMap =
  ...
  { /** 打开开发工具           */[name in PARSE_NAME["openDevTools"]]: []; } &
  { /** 主窗口_最小化          */[name in MAIN_WINDOW["min"]]: []; } &
  { /** 主窗口_最大化          */[name in MAIN_WINDOW["max"]]: []; } &
  { /** 主窗口_切换最小化      */[name in MAIN_WINDOW["min_toggle"]]: []; } &
  { /** 主窗口_切换隐藏        */[name in MAIN_WINDOW["hide_toggle"]]: []; } &
  { /** 主窗口_关闭            */[name in MAIN_WINDOW["close"]]: [boolean]; } &
  { /** 主窗口_退出            */[name in MAIN_WINDOW["quit"]]: []; } &
  { /** 主窗口_聚焦            */[name in MAIN_WINDOW["focus"]]: []; } &
  ...
  { /** 主窗口_清除歌词         */[name in MAIN_WINDOW["clear_lyric"]]: []; } &
  ...
  { /** 主窗口_清除歌曲地址     */[name in MAIN_WINDOW["clear_music_url"]]: []; } &
  ...
  { /** 主窗口_退出更新         */[name in MAIN_WINDOW["quit_update"]]: []; } &
...
export type MainHandleDataMap =
  ...
  { /** 热键_取状态                 */[name in HOT_KEY["status"]]: ParamRet<[], LxMusic.HotKey.HotKeyMap>; } &
  ...
  { /** 主窗口_取热键信息           */[name in MAIN_WINDOW["get_hot_key"]]: ParamRet<[], LxMusic.Common.DefaultHotKey>; } &
  ...
  { /** 主窗口_取配置               */[name in MAIN_WINDOW["get_setting"]]: ParamRet<[], LxMusic.Renderer.MainSettingInfo>; } &
  { /** 主窗口_清除缓存             */[name in MAIN_WINDOW["clear_cache"]]: ParamRet<[], void>; } &
  { /** 主窗口_取缓存大小           */[name in MAIN_WINDOW["get_cache_size"]]: ParamRet<[], number>; } &
  { /** 主窗口_取数据路径           */[name in MAIN_WINDOW["get_data_path"]]: ParamRet<[], string>; } &
  { /** 主窗口_取环境参数           */[name in MAIN_WINDOW["get_env_params"]]: ParamRet<[], NodeJS.Global["envParams"]["cmdParams"]>; } &
  ...
  { /** 主窗口_取用户网络接口列表   */[name in MAIN_WINDOW["get_user_api_list"]]: ParamRet<[], LxMusic.UserApi.ApiInfo[]>; } &
  { /** 主窗口_取用户网络接口状态   */[name in MAIN_WINDOW["get_user_api_status"]]: ParamRet<[], LxMusic.UserApiEvent.InitInfo>; } &
  { /** 歌词窗口_获取歌词配置       */[name in LYRIC_WINDOW["get_lyric_config"]]: ParamRet<[], LxMusic.WinLyric.LyricConfigInfo>; };

/** 回调函数 */

// @ts-ignore
export type MainCallback<PARAM, RET> = (event: Electron.IpcMainEvent, ...args: PARAM) => RET;
...
/** 主要发送 */
export type MainSend = <T extends keyof MainDataMap>(window: Electron.BrowserWindow, name: T, ...args: MainDataMap[T]) => void;
...
/** 面板发送 */
export type RendererSend = <T extends keyof MainDataMap>(name: T, ...args: MainDataMap[T]) => void;

/** 面板调用 */
export type RendererInvoke = <T extends keyof MainHandleDataMap>(name: T, ...args: MainHandleDataMap[T]["PARAM"]) => Promise<MainHandleDataMap[T]["RET"]>;
